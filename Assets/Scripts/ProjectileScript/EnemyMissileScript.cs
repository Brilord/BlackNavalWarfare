...
public class EnemyMissileScript : MonoBehaviour
{
    ...
    {
        PlayerScript player = collision.gameObject.GetComponent<PlayerScript>(); // Assume a PlayerScript is attached to player
        if (player != null)
        {
            player.TakeDamage(impactDamage); // Deal fixed impact damage to the player
        }
